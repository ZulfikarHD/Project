...
<template>
  <AppLayout>
    ...
    <div class="absolute top-0 w-full h-[30vh]">
      <img src="" alt="tools" class="w-full h-full object-cover rounded-lg shadow-lg" />
    </div>

    <!-- Hero Image Overlay -->
    <div class="absolute top-0 w-full h-[30vh] bg-black bg-opacity-60"></div>

    <!-- Search Lapangan -->
    <div class="relative z-10 mt-[15vh] container mx-auto px-4 sm:px-6 lg:px-8">
      <!-- Card Search -->
      ...
      <div class="flex justify-between gap-4 flex-wrap">
        ...
        <div>
          ...
          <div class="flex items-center mt-2 leading-tight">
            ...
            <p class="text-sm text-slate-600">08.00 - 21.00 (setiap hari)</p>
          </div>
        ...
        <div class="flex flex-wrap gap-6 items-start">
          <div class="flex gap-2 items-center mt-1.5">
            <a href="#" class="relative border border-green-600/70 px-2 py-0.5 rounded-md overflow-clip flex gap-2">
              <div class="bg-green-500 rounded-xl -left-1 inset-y-0 h-[20px] w-[28px] z-0 ml-0.5 absolute"></div>
              <img src="/svg/soccer-ball.svg" class="absolute w-4 h-4 left-0 ml-1 fill-green-50" />
              <span class="text-xs pl-6 text-green-800">Futsal</span>
            </a>

            <a href="#" class="relative border border-green-600/70 px-2 py-0.5 rounded-md overflow-clip flex gap-2">
              <div class="bg-green-500 rounded-xl -left-1 inset-y-0 h-[20px] w-[28px] z-0 ml-0.5 absolute"></div>
              <img src="/svg/badminton.svg" class="absolute w-4 h-4 left-0 ml-1" />
              <span class="text-xs pl-6 text-green-800">Badminton</span>
            </a>
          </div>
        </div>
      </div>
    ...
    <!-- Jadwal Main -->
    <section class="max-w-6xl mx-auto mt-4 mb-6 px-4 lg:px-0 pb-6 border-b border-slate-300">
      <h1 class="font-bold text-xl">Jadwal Main</h1>
      <div class="flex justify-between items-center relative mt-4">
        <button @click="scrollLeft" class="absolute left-0 z-10 top-1/2 -translate-y-1/2 bg-white p-2 rounded-full shadow-md hover:bg-slate-100 transition-colors" aria-label="Scroll Left">
          <ChevronLeft class="w-6 h-6 text-slate-700"></ChevronLeft>
        </button>

        <div ref="carousel" class="flex gap-5 overflow-x-auto no-scrollbar scroll-smooth py-2 px-6">
          <template v-for="i in 7" :key="i">
            <VenueScheduleCard></VenueScheduleCard>
          </template>
        </div>

        <button @click="scrollRight" class="absolute right-0 z-10 top-1/2 -translate-y-1/2 bg-white p-2 rounded-full shadow-md hover:bg-slate-100 transition-colors" aria-label="Scroll Right">
    ...
    <!-- List Venue Section -->
    <section class="max-w-6xl mx-auto mt-4 mb-6 px-4 lg:px-0 pb-6 border-b border-slate-200">
      <div class="flex justify-between gap-6 flex-wrap bg-white flex-1 rounded-lg shadow-md drop-shadow-lg overflow-hidden w-full relative p-4">
        <div class="flex flex-col gap-4 w-full md:w-fit">
          <img src="" alt="field Pic" class="w-full min-w-0 md:min-w-96 h-48 bg-slate-400 rounded-lg shadow-lg" />

          <div class="lg:grid grid-cols-2 hidden gap-4">
            <img src="" alt="field Pic" class="w-48 h-24 bg-slate-400 rounded-lg shadow-lg" />
            <img src="" alt="field Pic" class="w-48 h-24 bg-slate-400 rounded-lg shadow-lg" />
          </div>
        </div>
        ...
        <div class="flex flex-col w-full flex-1 px-4">
          ...
          <div class="pt-4 pb-6 border-b border-slate-200 flex flex-col flex-wrap gap-4">
            <div class="flex justify-between gap-4 flex-wrap">
              <h1 class="font-bold text-xl">Waktu Main</h1>
              <button @click="toggleAccordion" class="text-blue-500 flex items-center gap-1">
                <span>Lihat Jadwal</span>
                <ChevronDown v-if="!isAccordionOpen" class="w-4 h-4" />
                <ChevronUp v-if="isAccordionOpen" class="w-4 h-4" />
              </button>
            </div>
            ...
            >
              <div v-if="isAccordionOpen" class="mt-4 overflow-hidden">
                <div class="flex justify-between gap-4 flex-wrap">
                  <input
                    type="date"
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 mt-4">
                  <template v-for="i in 12" :key="i">
                    <button class="rounded-lg px-4 py-2 border border-blue-800 shadow hover:bg-blue-100 transition-colors duration-200">
                      <p class="text-xs text-blue-900 font-semibold">{{ 7 + i }}:00 - {{ 8 + i }}:00</p>
                    </button>
                  </template>
                </div>
              </div>
          ...
          <div class="pt-4 pb-6 border-b border-slate-200 flex justify-between flex-wrap gap-4">
            <div class="flex justify-end w-full flex-wrap gap-4">
              <button class="bg-green-500 text-white font-bold py-2 px-4 rounded hover:bg-green-600 transition-colors duration-200">
                Prosess
              </button>
            </div>
          </div>

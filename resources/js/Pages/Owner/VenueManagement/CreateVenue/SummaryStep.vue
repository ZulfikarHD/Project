<template>
    <div class="bg-white shadow-md rounded-lg p-6 mb-6">
        <h2 class="text-2xl font-bold mb-4 text-gray-800">Summary & Confirmation</h2>

        <!-- Venue Information Summary -->
        <div class="mb-6 border-b pb-4">
            <h3 class="text-xl font-semibold mb-2">Venue Information</h3>
        ...
        <div class="mb-6 border-b pb-4">
            <h3 class="text-xl font-semibold mb-2">Facility Information</h3>
            <div v-if="venue.fields && venue.fields.length" v-for="(field, index) in venue.fields" :key="index" class="mb-4">
                <p class="text-gray-700"><strong>Field {{ index + 1 }}:</strong> {{ field.name || 'N/A' }}</p>
                <p class="text-gray-700"><strong>Sports:</strong> {{ getSportsNames(field.sports) || 'N/A' }}</p>
            ...
            <button @click="editSection('facilityInfo')" class="mt-4 text-blue-600 hover:text-blue-800 font-medium">Edit Facility Information</button>
        </div>

        <!-- Availability & Pricing Summary -->
        <div class="mb-6">
            <h3 class="text-xl font-semibold mb-2">Availability & Pricing</h3>
            <div v-if="venue.availability && venue.availability.length" v-for="(day, index) in venue.availability" :key="index" class="mb-4">
                <h4 class="font-semibold text-gray-800">{{ day.name || 'N/A' }}</h4>
            ...
            <button @click="editSection('availabilityPricing')" class="mt-4 text-blue-600 hover:text-blue-800 font-medium">Edit Availability & Pricing</button>
        </div>

        <!-- Submit Button -->
        <div class="flex justify-between mt-6">
            <button @click="previousStep" class="px-4 py-2 bg-gray-300 rounded-md hover:bg-gray-400 transition duration-200">Back</button>
            <button @click="submitVenue" class="px-4 py-2 bg-green-500 text-white rounded-md hover:bg-green-600 transition duration-200">Submit</button>
        </div>
    </div>
</template>

<template>
    <div class="bg-white shadow-md rounded-lg p-6 mb-6">
        ...
        <div v-for="(field, index) in venue.fields" :key="index" class="mb-6">
            ...
            <div class="mb-4">
                ...
                />
                <div v-if="field.image" class="mt-2">
                    <img
                        :src="field.imageUrl"
                        alt="Field Image"
...
// Sports list (can be dynamically loaded)
...
const removeField = (index) => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            venue.value.fields.splice(index, 1);
            Swal.fire(
                "Removed!",
                "The field/court has been removed.",
                "success"
            );
        }
    });
...
const addEquipment = (index) => {
    venue.value.fields[index].equipment.push({
        name: "",
        quantity: 1,
    });
};
...
const handleFieldImageUpload = (e, index) => {
    ...
    if (file) {
        venue.value.fields[index].image = file;
        venue.value.fields[index].imageUrl = URL.createObjectURL(file); // Create a preview URL
    }
};
...
const nextStep = () => {
    ...
    if (
        venue.value.fields.some(
            (field) => !field.name || field.sports.length === 0
        )
    ) {
        Swal.fire({
            icon: "error",
